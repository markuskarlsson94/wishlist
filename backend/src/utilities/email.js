...
export const sendPasswordResetEmail = async (receiver, token) => {
	...
	await sendEmail(
		...
		[receiver],
		"Password Reset",
		`<p>We have received a request to reset your password for your <a href=${url}>${process.env.VITE_APP_NAME}</a> account. Follow the link below to set a new password</p><a href=${link}>Reset your password</a><p>This link will expire shortly for safety reasons. You can safely ignore this email if you did not request a password reset.</p><p>You can not reply to this message.</p>`,
		(data) => {
